...
class ThreadRealmCommunication(threading.Thread):
    def __init__(self, chats, target_realm_address, target_realm_port):
        self.chats = chats
        self.chat = {}
...
class Chat:
    ...
    def proses(self,data):
        ...
        try:
            ...
            if (command=='auth'):
                ...
                return self.add_realm(realm_id, target_realm_address, target_realm_port, data)
            
            elif (command=='recvrealm'):
                realm_id = j[1].strip()
                target_realm_address = j[2].strip()
                target_realm_port = int(j[3].strip())
                return self.recv_realm(realm_id, target_realm_address, target_realm_port, data)
    ...
    def add_realm(self, realm_id, target_realm_address, target_realm_port, data):
        j = data.split()
        j[0] = "recvrealm"
        data = ' '.join(j)
        data += "\r\n"
        ...
        self.realms[realm_id] = ThreadRealmCommunication(self, target_realm_address, target_realm_port)
        result = self.realms[realm_id].sendstring(data)
        return result

    def recv_realm(self, realm_id, target_realm_address, target_realm_port, data):
        self.realms[realm_id] = ThreadRealmCommunication(self, target_realm_address, target_realm_port)
        return {'status':'OK'}
